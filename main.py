...
def menu_principal() -> int:
    ...
    return IntPrompt.ask("\n[bold green]Escolha uma opção[/bold green]", default=9)
...
def cadastrar_professor():
    ...
    try:
        ...
        console.print(f"  ✅ [green]Professor '{prof.nome}' cadastrado com sucesso![/green]")
        return prof
...
def matricular_aluno(cursos: list[Curso], alunos: list[Aluno]):
...
def desmatricular_aluno(cursos: list[Curso], alunos: list[Aluno]):
    if not cursos:
    ...
    # Filtrar apenas cursos que têm alunos matriculados
    ...
    # Selecionar aluno para desmatricular
    ...
    # Confirmação antes de desmatricular (boa prática!)
    console.print(Panel(
...
def adicionar_nota(cursos: list[Curso]):
    if not cursos:
        console.print("  ⚠️ [yellow]Nenhum curso disponível.[/yellow]")
        return
    ...
    for i, a in enumerate(curso.alunos, 1):
        ...
        console.print(f"  {i}. {a.matricula} - {a.nome} | Média atual: {media:.1f}")
    idx_aluno = IntPrompt.ask("  Selecione o aluno", default=1) - 1
...
def main():
    ...
    while True:
        ...
        if opcao == 1:
            ...
            if prof: professores.append(prof)
        elif opcao == 2:
            aluno = cadastrar_aluno()
            if aluno: alunos.append(aluno)
        elif opcao == 3:
            cadastrar_curso(professores)
        elif opcao == 4:
            matricular_aluno(GerenciadorCursos._cursos, alunos)
        elif opcao == 5:
            desmatricular_aluno(GerenciadorCursos._cursos, alunos)
        elif opcao == 6:
            adicionar_nota(GerenciadorCursos._cursos)
        elif opcao == 7:
            listar_cursos()
        elif opcao == 8:
            gerar_relatorio()
        elif opcao == 9:
            console.print(Panel("\n[bold green]👋 Obrigado por usar o Sistema de Cursos![/bold green]\n", border_style="green"))
            break
        else:
